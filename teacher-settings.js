...
function buildDutySettings(){
  ...
  return `<div class="settings-editor"><div class="settings-editor-toolbar no-print"><button class="btn btn-sm btn-outline-secondary" onclick="addDutyPlaceRow()">Satır Ekle</button><button class="btn btn-sm btn-primary" onclick="saveDutyMatrix()">Kaydet</button></div><div class="settings-editor-body"><p class="text-muted small">Sütunlar günleri, satırlar nöbet yerlerini gösterir. Nöbet yeri adını değiştirip kaydedebilir veya yeni satır ekleyebilirsiniz.</p><div class="settings-table-scroll settings-table-scroll-wide" data-scroll-hint="Yana kaydırın"><table class="table settings-matrix mb-0"><thead><tr><th>Nöbet Yeri</th>${days.map(d=>`<th>${d}</th>`).join('')}<th class="no-print">İşlem</th></tr></thead><tbody id="dutyMatrixBody">${rows}</tbody></table></div></div></div>`;
}
