...
function renderTasks(){
  ...
  const counts=[...new Set(allTasks.map(t=>t.kind||'Genel'))].sort((a,b)=>a.localeCompare(b,'tr')).map(kind=>{const isActive=(kindFilter&&(kindFilter===kind));return `<span class="soft-chip chip-clickable${isActive?' chip-active':''}" onclick="filterTasksByKind('${escapeInlineJsString(kind)}')">${escapeHtml(kind)}: ${allTasks.filter(t=>(t.kind||'Genel')===kind).length}</span>`;}).join('');
