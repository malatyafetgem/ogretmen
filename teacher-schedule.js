...
function buildTeacherSheet(){
  const f=scheduleFilters();
  // Kural: 1 saati bile olan herkes çarşaf listesinde yer alır (idareci/PDR dahil).
  // isSchedulableTeacher yalnızca boş gün/saat hesapları için kullanılır.
  const teachers=sortedTeachers().filter(t=>t&&teacherLessons(t.id).length>0&&(!f.teacherId||t.id===f.teacherId));
  const days=visibleScheduleDays(f), hours=visibleScheduleHours(f);
...
function buildScheduleEntryList(){
  const rows=visibleFilteredScheduleItems().sort((a,b)=>dayOrder(a.day)-dayOrder(b.day)||hourOrder(a.hour)-hourOrder(b.hour)).map(s=>{
    ...
    return `<tr><td>${escapeHtml(s.day)}</td><td>${escapeHtml(lessonHourLabel(s.hour))}</td><td>${teacherLink(t)}</td><td>${escapeHtml(s.className)}</td><td>${escapeHtml(displaySubjectName(s.subject))}</td><td>${escapeHtml(scheduleNoteText(s)||'—')}</td><td class="no-print table-actions"><button class="btn btn-sm btn-outline-primary" onclick="openScheduleModal('${escapeInlineJsString(s.id)}')"><i class="fas fa-pen"></i></button><button class="btn btn-sm btn-outline-danger" onclick="deleteSchedule('${escapeInlineJsString(s.id)}')"><i class="fas fa-trash"></i></button></td></tr>`;
  }).join('');
